...
export default function PickedUpScreen({ onChildPress }: Props) {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        const pickedUpChildren = allChildren.filter(
          (child) => child.attendance === "picked_up"
        );

        setChildren(pickedUpChildren);
        setLoading(false);
      },
      (error) => {
  ...
  if (loading) {
    return (
      <View style={styles.center}>
        <ActivityIndicator size="large" color="#75339B" />
        <Text>Laster hentede barn...</Text>
      </View>
    );
  }
  ...
  if (children.length === 0) {
    return (
      <View style={styles.center}>
        <Text style={styles.empty}>Ingen barn er markert som hentet</Text>
      </View>
    );
  }
